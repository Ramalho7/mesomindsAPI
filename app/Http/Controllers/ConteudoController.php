    ...
    public function store(StoreContent $request)
{
    ...
    $contentType = ContentType::firstOrCreate(
        ...
        [
            ...
            'criador' => $validated['criador'] = $user->id,
        ]
    );
    ...
    $conteudo->load(['image', 'contentType', 'contentTag', 'creator']);

    return response()->json([
        'success' => true,
        'conteudo' => $conteudo,
        'image_url' => $conteudo->image ? asset('storage/' . $conteudo->image->file_path) : null,
    ], 201);
}
}
